...
def analyze_supply_chain(df: pd.DataFrame, inventory_cols: list) -> dict:
    ...
    return {'n_samples': len(df), 'n_nodes': len(inventory_cols), 'mean_inventory': df[inventory_cols].mean().to_dict(), 'total_inventory': df[inventory_cols].sum(axis=1).mean()}
...
def plot_supply_chain(df: pd.DataFrame, inventory_cols: list, title: str, output_path: Path, plot: bool=False):
    ...
    for col in inventory_cols:
        ax.plot(df['timestamp'], df[col], label=col.replace('_', ' ').title(), linewidth=1.2, alpha=0.7)
    ax.set_xlabel('Time')
